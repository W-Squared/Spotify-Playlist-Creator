            ...
            if "spotify" in song_data:
                song = SongFinder(
                    ...
                    created_at=datetime.strptime(
                        song_data["track"]["created_at"], "%Y-%m-%dT%H:%M:%S.%fZ"
                    ),
...
class DeDuper:
    ...
    def dedupePlaylist(self, playlist_id):
        tracks = []
        ...
        if len(track_ids) != len(unique_track_ids):
            ...
            for i in range(0, len(unique_track_ids), batch_size):
                ...
                self.spotify.playlist_replace_items(playlist_id, [f"spotify:track:{track_id}" for track_id in batch])
        else:
...
class Updater:
    ...
    def updateShade45Playlist(self):
        Shade45PlaylistID = "1KeYBRRe26f3NoKL2RUUvE"
        Finder = SongFinder()
        Songs = Finder.Shade45()
        CurrentPlaylistsongs = self.GetCurrentPlaylistTracks(Shade45PlaylistID)
        for Song in Songs:
            if Song and Song not in CurrentPlaylistsongs:
                self.spotify.playlist_add_items(Shade45PlaylistID, [Song])
    ...
    def GetCurrentPlaylistTracks(self, playlist_id):
        ...
        while results["next"]:
            ...
            tracks.extend(results["items"])
        return [
            f"spotify:track:{track['track']['id']}"
            for track in tracks
